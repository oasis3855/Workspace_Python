...
Version:
    ...
    1.6.2 (2026/03/06) IMDb取得失敗時にブラウザ起動と手動入力による補完機能を追加
    1.6.3 (2026/03/06) IMDb制限検知後、セッション終了まで自動取得をスキップするフラグ管理を追加
Author:
    Google Gemini
"""
...
# --- IMDbの自動取得を諦めて手動モードを強制するフラグ ---
FORCE_MANUAL_IMDB: bool = False
# ==========================================
...
# 製作年のスクレイピング
# div.movie-details -> section.txt-block -> p.data
# 「 1985年製作／116分／PG12／アメリカ 」


def get_movie_detailed_info(movie_id: str) -> Tuple[str, str]:
    """映画.comの個別ページから「原題」と「製作年」を取得する。

    Args:
        movie_id (str): 映画.comの映画ID。

    Returns:
        Tuple[str, str]: (原題, 製作年)。見つからない項目は空文字を返す。
    """
    if not movie_id:
        return "", ""
    ...
    try:
        ...
        # 映画詳細情報の親要素（div.movie-details）からp.dataを探す
        # セレクタを厳密にする場合は soup.select_one('div.movie-details section.txt-block p.data')
        data_tag = soup.find('p', class_='data')
        if data_tag:
            full_text = data_tag.get_text()
            html_content = data_tag.decode_contents()

            # 1. 製作年の抽出（例: 「1985年製作」から「1985」を取得）
            year_match = re.search(r'(\d{4})年製作', full_text)
            if year_match:
            ...
            if title_match:
                raw_orig = title_match.group(1).strip()
                original_title = re.sub(r'<[^>]+>', '', raw_orig)

    except Exception as e:
        print(f"詳細情報取得エラー (ID: {movie_id}): {e}")

    return original_title, production_year
...
def get_imdb_rating(original_title: str, release_year: str) -> str:
    ...
    search_url = f"https://www.imdb.com/search/title/?title={encoded_title}&title_type=feature&release_date={start_year}-01-01,{end_year}-12-31"

    # --- 1. 自動取得の試行（フラグがFalseの場合のみ実行） ---
    if not FORCE_MANUAL_IMDB:
        headers = {
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                # 202 Accepted 等、200以外が返ってきたら「検知された」とみなしてフラグを立てる
                print(f"  [Info] IMDbサーバーから制限を受けました(Code: {res.status_code})。以降は手動モードへ移行します。")
                FORCE_MANUAL_IMDB = True

        except Exception as e:
            print(f"IMDb取得エラー ({original_title}): {e}")

    # --- 2. 手動入力（フラグがTrueの場合、または自動取得で確定できなかった場合） ---
    print(f"\n--- IMDb情報の補完 ---")
    print(f"対象映画: {original_title} ({release_year})")
    manual_choice = input("ブラウザで表示し、値を手動入力しますか？ (y/n) [n] > ").strip().lower()

    if manual_choice == 'y':
        print(f"ブラウザでIMDbを開いています...")
        webbrowser.open(search_url)
        # ユーザーにRatingの入力を促す
        manual_rating = input("ブラウザに表示されたRatingを入力してください (例: 7.5 / スキップはEnter) > ").strip()
        return manual_rating if manual_rating else "-"
...
def get_movie_info(cleaned_title: str, movie_cache: Dict[str, Dict[str, str]]) -> Dict[str, str]:
    ...
    if cleaned_title in movie_cache:
        ...
        # IDはあるが原題が欠けている場合、詳細ページを再取得して補完
        if info.get("ID") and not info.get("原題"):
            print(f"詳細情報をmovie_databaseより補完中: {cleaned_title}")
            orig_t, prod_y = get_movie_detailed_info(info["ID"])
            info["原題"] = orig_t
            if prod_y:  # 製作年が見つかれば更新
                info["公開年"] = prod_y
        return info
        ...
        if movie_items:
            for item in movie_items:
                ...
                if title_tag and time_tag:
                    ...
                    # 検索一覧から得られる年（リマスター版の可能性あり）
                    year_match = re.search(r'\d{4}', time_tag.get_text(strip=True))
                    fallback_year = year_match.group() if year_match else "不明"
                    rating = rating_tag.get_text(strip=True) if rating_tag else "-"

                    # 完全一致した場合の即時確定ロジック
                    if found_title == cleaned_title:
                        # 個別ページから「真の製作年」と「原題」を取得
                        orig_t, prod_y = get_movie_detailed_info(movie_id)
                        final_year = prod_y if prod_y else fallback_year
                    ...
                    candidates.append({
                        "映画名": found_title, "公開年": fallback_year,
                        "評価": rating, "ID": movie_id
                    })
        ...
        if candidates:
            ...
            while True:
                ...
                if choice.isdigit() and 1 <= int(choice) <= len(candidates):
                    ...
                    # ここでも個別ページへ飛んで詳細情報を取得
                    orig_t, prod_y = get_movie_detailed_info(target["ID"])
                    selected_data = {
                        "公開年": prod_y if prod_y else target["公開年"],
                        "評価": target["評価"],
                        "ID": target["ID"],
                        "原題": orig_t,
                        "IMDb": ""

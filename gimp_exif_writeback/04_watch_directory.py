...
"""指定ディレクトリ内の画像ファイル操作をリアルタイムで監視・検知するスクリプト。

このモジュールは、watchdog ライブラリを使用して特定のディレクトリ（デフォルト: デスクトップ）を
常時監視し、対象の拡張子（.jpg, .jpeg）を持つファイルの新規作成・移動・更新イベントをリアルタイムに検知します。
また、OSやアプリのファイル書き込み時に連続発生するイベントをデバウンス（間引き）処理し、
規定時間内（1秒以内）の重複検知を抑止してログ出力します。
...
def main() -> None:
    ...
    start_directory_monitoring(
        WATCH_DIRECTORY_PATH, TARGET_FILE_EXTENSIONS, DEBOUNCE_INTERVAL_SECONDS
    )
